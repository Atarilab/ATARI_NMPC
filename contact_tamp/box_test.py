...
from visualization.vis_tools import vis_box
...
if __name__ == "__main__":
    ...
    for c in contacts[2:]:  # initially p3 p4 in air
        for i in range(0, N_impact):
            solver.params[c.active.name][:, i] = 0.0
    for c in contacts[2:]:  # initially p3 p4 in air
    ...
    for i in range(N):
        com_pose = []
        for j in range(6):
            com_pose.append(s[j][i])
        com_trajectory.append(com_pose)
    timestep_trajectory = dt_.flatten().tolist()

    print("com_trajectory:", com_trajectory)

    vis_box(com_trajectory, timestep_trajectory)
