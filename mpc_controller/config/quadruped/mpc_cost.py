...
@dataclass
class G1TrotCyclicCost(MPCCostConfig):
    @staticmethod
    def __init_np(l : List, scale : float=1.):
        """ Init numpy array field."""
        return field(default_factory=lambda: np.array(l) * scale)
    ...
    W_cnt_f_reg: np.ndarray = __init_np([[1e-1, 1e-1, 1e-1,1e-1,1e-1,1e-1]] * N_EEFF)
    # Feet position constraint stability
    W_foot_pos_constr_stab: np.ndarray = __init_np([5e1] * N_EEFF)

    # Foot displacement penalization
    W_foot_displacement: np.ndarray = __init_np([1e1])
...
@dataclass
class G1StandCyclicCost(MPCCostConfig):
    @staticmethod
    def __init_np(l : List, scale : float=1.):
        """ Init numpy array field."""
        return field(default_factory=lambda: np.array(l) * scale)
